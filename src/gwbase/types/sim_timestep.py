...
class SimTimestep(BaseModel):
    ...
    def as_type(self) -> bytes:
        """
        Serialize to the sim.timestep.000 representation designed to send in a message.
...
class SimTimestep_Maker:
    ...
    @classmethod
    def type_to_tuple(cls, b: bytes) -> SimTimestep:
        """
        ...
        Raises:
           GwTypeError: if the bytes are not a sim.timestep.000 type

        Returns:
            SimTimestep instance
        """
        try:
            d = json.loads(b)
        except TypeError:
    ...
    def dict_to_tuple(cls, d: dict[str, Any]) -> SimTimestep:
        """
        Translates a dict representation of a sim.timestep.000 message object
